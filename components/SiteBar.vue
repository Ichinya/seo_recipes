      ...
      <div class="sticky top-0 -ml-0.5 pointer-events-none">
        ...
        <div class="bg-white dark:bg-slate-900 relative pointer-events-auto">
          <button type="button"
            ...
            Быстрый поиск...<span class="ml-auto pl-3 flex-none text-xs font-semibold">Ctrl K</span></button>
        </div>
        <div class="h-8 bg-gradient-to-b from-white dark:from-slate-900"></div>
      </div>
      <ContentNavigation v-slot="{ navigation }">
        <ul>
          <div v-for="nav of navigation" :key="nav._path">
            <MenuLinkList v-if="nav.children" :links="nav"/>
            <MenuLinkOne v-else :link="nav"/>
          </div>
        </ul>
      </ContentNavigation>
    </nav>
